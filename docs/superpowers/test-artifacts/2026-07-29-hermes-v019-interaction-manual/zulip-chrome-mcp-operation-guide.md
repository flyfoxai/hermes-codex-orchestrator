...
- 完整新增经验和本轮索引见 `../2026-07-29-hermes-v019-interaction-fix-manual/zulip-chrome-mcp-operation-guide.md`。
- prompt 消费后会从 Zulip API 消息集合消失；必须在首次点击前记录 prompt ID、实际按钮标签、enabled 状态和截图，不能事后依赖 API 恢复 zform。
- 完全相同的 URL 只会激活原标签。双标签竞态可在 origin 后增加唯一 query 参数强制创建第二标签；完成后关闭它并恢复单标签。
- prompt 最终删除不等于 stale-tab 重复消费已被拒绝。本轮 `690/691 -> 692/693` 中 A、B 两次点击都被接受并分别确认，属于明确 FAIL。
- PC-05/06 应先在当前话题创建一个无副作用的只读工作，再查询该话题的真实 work/objective。Zulip 回复之外，还要只读核对 call、submission 和 objective execution 的后端终态。
- Android 验收必须使用真实 Zulip Android App 或可控模拟器。没有 adb、设备和 App 版本信息时记为 `NOT RUN / BLOCKED`，不能用移动视口网页替代。
