...
export function createAcl({ admins, projects } = {}) {
  if (!Array.isArray(admins) || !Array.isArray(projects) || admins.some((id) => !positive(id))) {
    throw stateError("ACL_CONFIG_INVALID", "ACL configuration is invalid.");
  }
  const adminSet = new Set(admins);
  const grants = new Map();
  for (const project of projects) {
    if (typeof project?.projectId !== "string" || !project.acl) throw stateError("ACL_CONFIG_INVALID", "ACL configuration is invalid.");
    const roles = new Map();
    for (const [list, role] of [["viewers", "viewer"], ["contributors", "contributor"], ["maintainers", "maintainer"]]) {
      if (!Array.isArray(project.acl[list])) throw stateError("ACL_CONFIG_INVALID", "ACL configuration is invalid.");
      for (const userId of project.acl[list]) {
        if (!positive(userId)) throw stateError("ACL_CONFIG_INVALID", "ACL configuration is invalid.");
  ...
  function roleFor(input) {
    if (!exact(input, ["userId", "projectId"]) || !positive(input.userId) || typeof input.projectId !== "string") {
      throw stateError("ACL_REQUEST_INVALID", "ACL request is invalid.");
    }
  ...
  function requirePermission(input) {
    ...
    if (input.projectId === null) {
      if (!adminSet.has(input.userId)) throw stateError("ACL_FORBIDDEN", "Permission denied.");
    ...
    if (!minimum) throw stateError("ACL_REQUEST_INVALID", "ACL request is invalid.");
